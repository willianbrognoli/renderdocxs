...
class Builder:
    ...
    @classmethod
    def _monta_cabecalho(cls, cabecalho, corpo):
        """Devolve (linhas_da_caixa, corpo_ajustado).

        Múltipla escolha (corpo contém alternativas A–E): o enunciado —
        linhas do corpo antes da 1ª alternativa — sobe para a caixa; só as
        alternativas ficam fora. Certo/Errado: a caixa fica com número +
        banca + comando (quando houver) e a afirmativa fica fora."""
        ...
        if resto or corpo:
            if not corpo and resto:
                # tudo veio no cabecalho: 1ª frase fica na caixa se for comando
                caixa = [cab]
                m = re.match(r'([^.?!:]*[.?!:])\s*(.*)', resto, re.S)
                if m and cls._COMANDO_RE.search(m.group(1)):
                    caixa = [(cab + ' ' + m.group(1)).strip()]
                    r2 = m.group(2).strip()
                    corpo = [r2] if r2 else []
                else:
                    corpo = [resto]
    ...
    def questao(self, cabecalho, corpo, certo_errado=False):
        ...
        linhas_caixa, corpo = self._monta_cabecalho(cabecalho, corpo)
        for lc in linhas_caixa:
            self.add(retext_para(self.f.q_cab, lc, highlight_fgv=False))
        for linha in corpo:

...
def _retag_font(data, family, sub, bold, italic):
    ...
    if 'OS/2' in tables:
        ...
        # fsType: LibreOffice recusa embeds "Preview & Print" (4) e
        # "Restricted" (2); normaliza a cópia embutida para Editable (8)
        data[t + 8:t + 10] = struct.pack('>H', 8)
...
def _set_fstype_editable(data):
    """Normaliza o fsType (OS/2, offset 8) da cópia embutida para Editable (8):
    LibreOffice recusa fontes embutidas com licença Preview&Print/Restricted."""
    data = bytearray(data)
    try:
        num = struct.unpack('>H', data[4:6])[0]
        for i in range(num):
            ...
            if data[off:off + 4] == b'OS/2':
                ...
                data[toff + 8:toff + 10] = struct.pack('>H', 8)
                break
    except Exception:
        pass
    return bytes(data)
    ...
    for caminho in arquivos:
        ...
        for slot in slots:
            ...
            if merged:
                ...
                base = _set_fstype_editable(raw0)
            raw = bytearray(base)

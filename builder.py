...
class Builder:
    ...
    def _scale_txbx(art, scale):
        """Reduz proporcionalmente a fonte (w:sz/w:szCs) e o entrelinhas
        exato (w:spacing w:line) apenas DENTRO das caixas de texto da capa,
        sem tocar no restante da arte."""
        if scale >= 0.999:
            return art
        ...
        def shrink_box(m):
            ...
            def sz(mm):
                val = max(4, int(round(int(mm.group(2)) * scale / 2.0)) * 2)
                return '<w:%s w:val="%d"/>' % (mm.group(1), val)

            box = re.sub(r'<w:(sz|szCs) w:val="(\d+)"/>', sz, box)
            ...
            box = re.sub(r'<w:spacing[^>]*w:line="(\d+)"[^>]*'

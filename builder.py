...
class Builder:
    ...
    def para_dourado(self, text):
        # parágrafo da APRESENTAÇÃO no dourado da marca (9C7A21)
        p = retext_para(self.f.body_p, text)
        # injeta cor dourada em todos os runs de texto do parágrafo
        p = re.sub(r'(<w:rPr>)(?!.*?<w:color)', r'\g<1><w:color w:val="9C7A21"/>', p)
        # runs sem rPr: adiciona rPr com a cor
        p = re.sub(r'(<w:r>)(<w:t)', r'\g<1><w:rPr><w:color w:val="9C7A21"/></w:rPr>\g<2>', p)
        self.add(p)
    ...
    def mnemonico(self, titulo, texto):
        ...
        box = box.replace('<w:color w:val="7A2E2E"', '<w:color w:val="000000"')
        # rótulo do box passa a ser "MNEMÔNICO/ BIZU" (evita duplicar se já estiver trocado)
        if 'MNEMÔNICO/ BIZU' not in box:
            box = box.replace('MNEMÔNICO', 'MNEMÔNICO/ BIZU')
        self.add(box)
        self.blank()
    ...
    for cap in data.get('capitulos', []):
        ...
        for blk in cap.get('blocos', []):
            ...
            if t == 'paragrafo':
                ...
                pass  # tabela "O QUE ESTUDEI" removida: não é mais gerada
            elif t == 'imagem':

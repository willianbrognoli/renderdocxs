...
class Builder:
    ...
    # ---------- estrutura ----------
    # v7.2: capa com título longo. As duas shapes do título (faixa escura e
    # caixa de texto) têm altura fixa no template, dimensionada para 2 linhas
    # a 22pt; títulos de 3+ linhas saíam cortados. Estima-se o número de
    # linhas e as shapes crescem o necessário, mantendo o centro vertical.
    _COVER_CHARS_POR_LINHA = 28   # calibrado na largura útil da caixa a 22pt
    _COVER_EMU_POR_LINHA = 350000  # ~0,97 cm por linha extra (22pt + folga)

    @classmethod
    def _cover_linhas(cls, titulo):
        limite = cls._COVER_CHARS_POR_LINHA
        linhas, atual = 1, ''
    ...
    def cover(self, titulo):
        ...
        linhas = self._cover_linhas(titulo)
        if linhas > 2:
            art = self._cover_grow(art, (linhas - 2) * self._COVER_EMU_POR_LINHA)
        self.add(art)

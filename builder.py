    ...
    **negrito**  __negrito sublinhado__  %%negrito vermelho%%
    ...
    for part in _TOKEN_RE.split(_sanitize(text)):
        ...
        if part.startswith('**') and part.endswith('**') and len(part) > 4:
            txt = part[2:-2]
            # símbolo puro nunca sai em negrito
            rpr = base_rpr if _is_symbol_only(txt) else bold_rpr
...
class Builder:
    ...
    @staticmethod
    def _scale_txbx(art, scale):
        """Reduz proporcionalmente o título da capa para caber na faixa.
        Caminho 1: quando os runs da caixa têm tamanho explícito (w:sz/w:szCs),
        reduz esses valores e o entrelinhas exato. Caminho 2 (fallback): quando
        a fonte vem herdada do estilo e NÃO há w:sz dentro da caixa, aplica
        <a:normAutofit fontScale> no bodyPr da forma, que escala o texto
        renderizado no Word e no LibreOffice/Stirling sem precisar conhecer o
        tamanho base. Nunca aplica os dois ao mesmo tempo (evita escala dupla)."""
        if scale >= 0.999:
            return art
        ...
        art = re.sub(r'<w:txbxContent>.*?</w:txbxContent>', shrink_box,
                     art, flags=re.S)
        if sz_hits[0]:
            return art

        # fallback: fonte herdada do estilo -> escala via bodyPr normAutofit
        naf = ('<a:normAutofit fontScale="%d" lnSpcReduction="%d"/>'
               % (int(round(scale * 100000)),
                  min(20000, int(round((1.0 - scale) * 100000)))))

        def bodypr_self(mm):
            return '<wps:bodyPr%s>%s</wps:bodyPr>' % (mm.group(1), naf)

        def bodypr_full(mm):
            inner = re.sub(r'<a:(?:noAutofit|spAutoFit|normAutofit)'
                           r'[^>]*/>', '', mm.group(2))
            return '<wps:bodyPr%s>%s%s</wps:bodyPr>' % (mm.group(1), naf,
                                                        inner)

        art = re.sub(r'<wps:bodyPr([^>]*)/>', bodypr_self, art)
        art = re.sub(r'<wps:bodyPr([^>]*[^/])>(.*?)</wps:bodyPr>',
                     bodypr_full, art, flags=re.S)
        return art
